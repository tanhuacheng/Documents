/*
 * File Name    : sinosims-inc.h
 * Author       : Tanhuacheng @ sinosims
 * Version      : V0.0.1
 * Data         : Apr 6, 2015
 * Description  : File operations for Sinosims USB driver
*/
...
struct usb_sinosims {
    ...
    struct urb *bulk_in_urb;            /* the usb to read data with */
...
#define to_sinosims_dev(d) container_of(d, struct usb_sinosims, kref)

static struct usb_driver sinosims_driver;
static void sinosims_draw_down(struct usb_sinosims *dev);

static void sinosims_delete(struct kref *kref)
{
...
static const struct file_operations sinosims_fops = {
    .owner =    THIS_MODULE,
    .read =     sinosims_read,
